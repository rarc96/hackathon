...
# === FALSE NEGATIVE (MISSED MANUAL) ANALYSIS ===
false_neg = df[(df['Manual_Actual'] == 1) & (df['Algo_Manual'] == 0)]
print(f"Total false negatives (after cleaning): {len(false_neg)}\n")
...
cols_exist = [c for c in out_cols if c in false_neg.columns]

display_cols = false_neg[cols_exist].copy()
display_cols['Expected_Algo_Decision'] = 'Manual'

print("\nFalse negatives – risk scores and reasons (head 20):")
print(display_cols.head(20))

# Export to Excel for audit/review
display_cols.to_excel("false_negatives_risks_and_reasons.xlsx", index=False)
print("\nDetailed false negatives saved to: false_negatives_risks_and_reasons.xlsx")
